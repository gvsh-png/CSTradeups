...
export default function SavedTradeUps({ items, onRemove, onUpdate }: SavedTradeUpsProps) {
      ...
      {items.map((item) => (
        <div key={item.id} className="relative group">
          ...
          <button
            ...
            className="absolute top-3.5 right-[6.5rem] p-1.5 rounded border border-[var(--border)] text-[var(--text-muted)] hover:text-[var(--loss)] hover:border-[var(--loss)]/30 sm:opacity-0 sm:group-hover:opacity-100 transition-all duration-150"
            title="Remove"
          >
